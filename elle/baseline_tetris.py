#!/usr/bin/env python3
"""
Baseline controller for three Tetris goals
H0 -> R1
"""
...
class BaselineTetris:
    ...
    def belief_update(self, s_before: MbagObs, a_h_obs: Action, is_anchor: bool) -> None:
        print("\n==================== BASELINE BELIEF UPDATE ====================")
        print("Observed human action:", a_h_obs)
        print("Prior belief:", self.state.belief)

        # Anchor is uninformative: belief unchanged, but cache must be populated
        if is_anchor:
        ...
        # Use H0 likelihood instead of H2
        logP_H0 = self._log_pi_H0(s_before, A_h)
        idx = A_h.index(a_h_obs)

        log_post = self.state.log_weights + logP_H0[:, idx]
        ...
        # Cache robot actions under updated belief
        A_r, _, Qpg = self._pi_R1(s_before, a_h_obs, self.state.belief)
        self._cached_A_r = A_r
        self._cached_Q_per_goal = Qpg
